...
const filterByNameAndType = function(state, searchText, searchType, activePage = 0, itemsCountPerPage = 0) {
  
  let start = (activePage - 1) * itemsCountPerPage
  let end = start + itemsCountPerPage
  let arr = state.pokemons.filter(item => 
    item.name.includes(searchText) &&
    (searchType === '' || item.types[0].type.name === searchType )
  )
  if (itemsCountPerPage !== 0 ) arr = arr.slice(start, end)
...
export const getPokemonsFilteredByNameAndType = createSelector(
  [ filterByNameAndType ],
  (filterByNameAndType) => {return filterByNameAndType;}
)

const pokemonsTypes = function(state) {

  let arr = []
  return state.pokemons.filter(item => {
...
export const getPokemonTypes = createSelector(
  [ pokemonsTypes ],
  (pokemonsTypes) => {return pokemonsTypes}
)